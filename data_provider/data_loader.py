...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        '''
        Read in the data from the file. Then cut it into train, validation and test. 
        Note that the train, validation and test set will then be further broken down into smaller sequences.
        And each sequence will be the data unit.

        If scale, then the train set will be scaled using a standard scaler.
        ---
        ...
        # cutting the entire dataset, which is a continuous time series, into train, validation and test
    ...
    def __len__(self):
        return (len(self.data_x) - self.seq_len - self.pred_len + 1) * self.enc_in # tot_len * feat_num
